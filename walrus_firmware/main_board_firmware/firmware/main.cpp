...
char temp_addr[10][8] = {TEMP_1_ADDR, TEMP_2_ADDR, TEMP_3_ADDR, TEMP_4_ADDR, TEMP_5_ADDR, TEMP_6_ADDR, TEMP_7_ADDR, TEMP_8_ADDR, TEMP_9_ADDR, TEMP_10_ADDR};
...
void setup()
{
	...
	upper[0].begin(i2c_bus_0);
	upper[1].begin(i2c_bus_1);
	upper[2].begin(i2c_bus_2);
	upper[3].begin(i2c_bus_3);
	lower[0].begin(i2c_bus_4);
	lower[1].begin(i2c_bus_5);
	lower[2].begin(i2c_bus_6);
	lower[3].begin(i2c_bus_7);
...
{	
	...
	if (counter % (TEMP_READ_RATE/ROS_MSG_RATE) == 0)
	{
		...
		for (int l = 0; l < 10; l++)
		{
			if (temp_en[l])
			{
				...
				for (int m = 0; m < 9; m++)
					temp_buff = exttemp_sense.read();
				temp[l] = (temp_buff[1] << 8) | temp_buff[0];
			}
		}
	...
	for (int l = 0; l < 4)
	{
		...
		rx_msg.ucell_shutdown[] = (upper[l].getShutdown() ? 1 : 0);
		...
		rx_msg.lcell_shutdown[] = (lower[l].getShutdown() ? 1 : 0);
	}
	//Publish our message
	rx.publish(&rx_msg);
