angular.module("app").controller("BoomControlToolCtrl",
...
angular.module("app").directive("positionEffortControlToolChannel", function() {
	...
	controller: function($scope, roslib, $interval) {
	    ...
	    var command_pub = roslib.advertise($scope.controllerNamespace+"/command", "position_effort_controller/PositionEffortCommand");
		...
		if($scope.enable) {
			...
			command_pub.publish({set_point: $scope.value, mode: PositionEffortCommand.POSITION});
		    }
		    else {
			command_pub.publish({set_point: $scope.value, mode: PositionEffortCommand.EFFORT});
		    }
		}
		else {
		    command_pub.publish({mode: PositionEffortCommand.HOLD_POSITION, set_point: 0}); // setpoint is ignored, but needed cause serialization raises error otherwise
		}
	    }
	...
	templateUrl: "/tools/position-effort-control-tool-channel.tpl.html"
    };
});
